...
class QueueManager:
    ...
    def _evaluate_cache_state(self, file_path: Path, fingerprint: str, file_size: int) -> tuple[bool, bool, dict]:
        ...
        if existing.get('is_organized', 0) == 1:
            ...
            # Si el archivo está en su sitio original y no ha cambiado...
                ...
                logger.info("[OK]  Contenido reconocido (Fingerprint + MTime) y completamente organizado.")
    ...
    def _process_file(self, task: FileTask) -> None:
        ...
        logger.info(f"[OK]  Hash ED2K calculado en {elapsed_str}: {ed2k_hash}")
        ...
        if force_rehash and existing and ed2k_hash == existing['ed2k_hash']:
            ...
            logger.info(f"[INFO]  Fecha de registro actualizada para {file_name}. Proceso optimizado finalizado.")
        ...
        self._db.save(file_path, task.file_size, fingerprint, ed2k_hash, ed2k_link)
        logger.info("[OK]  Hash guardado inicialmente en BBDD.")
