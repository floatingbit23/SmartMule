...
# Separamos la logica de extracción en funciones individuales (ZIP nativo vs otros formatos (RAR, 7Z, etc.))
...
def _calculate_verdict(path_name: str, expected_type: str, detected_media: Optional[str], 
    ...
    if has_dangerous and expected_type not in ["software", "games"]:
        logger.critical(f"[MALWARE] [Inspector] ¡SUPLANTACIÓN! {path_name} (esperado {expected_type}) contiene ejecutables.")
    ...
    if detected_media:
        logger.info(f"[i] [Inspector] Detectado contenido principal de tipo: {detected_media}")
    else:
        logger.warning("[WARN] [Inspector] No se detectó contenido multimedia válido en el contenedor.")
...
def inspect_archive(filepath: str, expected_type: str = "unknown") -> dict:
    ...
    try:
        ...
        # Manejo de errores de extracción (archivos ZIP cifrados o corruptos)
            ...
            logger.warning(f"[i] [Inspector] Archivo ZIP cifrado: {path.name}")
        ...
        # Captura de errores de patool o archivos corruptos/cifrados
            ...
            if any(word in err_msg for word in ["password", "encrypt", "checksum"]):
                ...
                logger.warning(f"[i] [Inspector] Archivo cifrado/corrupto ({ext}): {path.name}")
                return {"status": "SUSPICIOUS", "detected_media": None}
    ...
    # Si fallo crítico en la función inspect_archive
        ...
        logger.error(f"[ERR] [Inspector] Fallo crítico en {path.name}: {e}")
        return {"status": "ERROR", "detected_media": None}
