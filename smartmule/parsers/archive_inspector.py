...
# Aplicamos el principio de Responsabilidad ?nica (Single Responsibility Principle)
# Separamos la l?gica de extracci?n de la l?gica de análisis
...
def _list_generic(filepath: str) -> list[str]:
    ...
    # patool imprime a stdout, as? que lo capturamos
...
def _analyze_file_list(file_list: list[str]) -> tuple[Optional[str], list[str]]:
    ...
    for fname in file_list:
        ...
        # 1. Comprobamos si es una extensi?n peligrosa
...
def _calculate_verdict(path_name: str, expected_type: str, detected_media: Optional[str], 
    ...
    """Aplica la l?gica de seguridad y devuelve el veredicto final."""

    has_dangerous = len(dangerous_files) > 0
    
    # 1. CASO CR?TICO: Suplantaci?n (Ejecutables donde no deber?a haberlos)
    if has_dangerous and expected_type not in ["software", "games"]:
        logger.critical(f"[MALWARE] [Inspector] ?SUPLANTACI?N! {path_name} (esperado {expected_type}) contiene ejecutables.")
        return {"status": "MALICIOUS", "detected_media": "software", "representative": dangerous_files[0]}

    # 2. CASO CR?TICO: Malware en documentos
    if has_dangerous and expected_type == "documents":
        logger.critical("[MALWARE] [Inspector] ?MALWARE! Documento contenedor de scripts detectado.")
        return {"status": "MALICIOUS", "detected_media": "software"}

    # 3. CASO INFORMATIVO: Software leg?timo
    if has_dangerous:
        logger.info(f"[OK] [Inspector] Ejecutables en contenedor de {expected_type}. Permitido por contexto.")
    else:
        logger.info(f"[OK] [Inspector] Contenedor limpio. Contiene {len(file_list)} elementos.")

    if detected_media:
        logger.info(f"? [Inspector] Detectado contenido principal de tipo: {detected_media}")
    else:
        logger.warning("[WARN] [Inspector] No se detect? contenido multimedia válido en el contenedor.")
...
def inspect_archive(filepath: str, expected_type: str = "unknown") -> dict:
    ...
    """
    Orquestador de la inspecci?n de archivos comprimidos. Extrae la lista de archivos y delega el análisis de seguridad.
    ...
    try:
        ...
        # Paso 1: Obtenci?n de la lista de archivos
        try:
        ...
        # Manejo de errores de extracci?n (archivos ZIP cifrados o corruptos)
        except PermissionError:

            # Captura de ZIPs con contrase?a
        ...
        # Captura de errores de patool o archivos corruptos/cifrados
            ...
            # Identificamos si es por cifrado/corrupci?n o un error genérico
            ...
            logger.error(f"[ERR] [Inspector] Error procesando {path.name}: {e}")
    ...
    # Si fallo cr?tico en la funci?n inspect_archive
    except Exception as e:
        logger.error(f"[ERR] [Inspector] Fallo cr?tico en {path.name}: {e}")
        return {"status": "ERROR", "detected_media": None}
