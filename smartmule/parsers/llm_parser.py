...
# System Prompt base, enfocado en estructuraci?n dura sin inventarse datos

SYSTEM_PROMPT = """Eres un experto en extracci?n de metadatos de archivos de internet. 
...
Reglas:
1. Elimina etiquetas in?tiles: x264, x265, HEVC, AC3, HDRip, WEB-DL, Dual, Spanish, Castellano, subs, uploader name, by mDudikoff, etc.
2. Identifica correctamente la calidad ("quality") si está presente (1080p, 720p, 4K, 2160p, UHD, 480p).
3. Detecta "season" y "episode" si es una serie. Usa n?meros enteros.
4. Identifica "year" si existe. Usa n?mero entero.
...
# Funci?n principal que recibe el nombre y opcionalmente contexto técnico de Regex
def parse_with_llm(filename: str, context: dict = None) -> dict:

    """
    Intenta limpiar y extraer toda la informaici?n del archivo usando Inteligencia Artificial.
    Es el paso "Capa 2" de nuestro pipeline si Regex falla (baja confianza) o la entropia es alta.
    Se le puede pasar un diccionario 'context' con datos ya extra?dos por Regex para ayudar.
    """
...
def _call_gemini(filename: str, extra_context: str = "") -> dict:
    """Llama a la nube usando Gemini (v?a SDK google-genai)."""
...
def _call_local_llm(filename: str, extra_context: str = "") -> dict:
    ...
    if not LMSTUDIO_API_KEY:
         logger.warning("[WARN] LMSTUDIO_API_KEY no definida, enviando vac?o...")
    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                # Fusionamos el System Prompt con el User Prompt para m?xima compatibilidad
        ...
        try:
            ...
            if start_index != -1 and end_index != -1:
                ...
                # Si no hay llaves, quiz?s el modelo respondi? con bloques markdown
                if "```" in result_str:
