...
class ColoredFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        # Lógica de colores por emojis si no es crítico (ya que ya tiene color violeta)
            ...
            elif msg_str.startswith("[ERR]"):
                log_message = f"{self.RED}{log_message}{self.RESET}"
            elif msg_str.startswith("[WARN]"):
...
def validate_paths() -> bool:
    ...
    if not INCOMING_PATH.exists():
        logger.error(
            f"[ERR]  La carpeta Incoming no existe: {INCOMING_PATH}. "
            f"Verifica que eMule está instalado y la ruta sea correcta en .env"
        )
        return False

    if not INCOMING_PATH.is_dir():
        logger.error(
            f"[ERR]  La ruta Incoming no es un directorio: {INCOMING_PATH}"
        )
        return False
