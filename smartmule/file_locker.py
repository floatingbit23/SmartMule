...
def is_file_locked(path: Path) -> bool:
    ...
    if path.is_dir():
        ...
        try:
            ...
            logger.warning(f"[WARN]  Error detectando archivos en carpeta '{path.name}': {e}")
            return True
    else:
    ...
    while True:
        ...
        if not is_file_locked(path):
            ...
            if attempt > 1: # Si ha habido reintentos, informo del tiempo total transcurrido
                ...
                logger.info(
                    f"[OK] {item_type} '{path.name}' desbloqueado tras {attempt} intento(s) ({elapsed:.1f}s)"
                )
            
            else: # Si no ha habido reintentos, informo de que el archivo está accesible inmediatamente
                logger.debug(f"[OK] {item_type} '{path.name}' accesible inmediatamente!")
            return True
        ...
        if elapsed + current_delay >= timeout:
            logger.error(
                f"[ERR]  TIMEOUT: No pude acceder a {item_type.lower()} '{path.name}' tras {timeout}s ({attempt} intentos)."
            )
            return False
        ...
        logger.log(
            log_level,
            f"[WARN]  {item_type} '{path.name}' bloqueado (intento #{attempt}). "
            f"[INFO]  Reintentando en {current_delay:.1f}s..."
        )
